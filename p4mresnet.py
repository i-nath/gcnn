...
def upsample_p4m_feature_map(x: Tensor, size: tuple[int, int]) -> Tensor:
    """Upsample only the spatial dimensions of a P4M feature map."""
...
class P4MZ2ConvBlock(nn.Module):
...
class P4MP4MConvBlock(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.weight = nn.Parameter(
            torch.empty(out_channels, in_channels, 8, kernel_size, kernel_size)
        )
        self.bias = nn.Parameter(torch.zeros(out_channels,)) if bias else None
    ...
    def forward(self, x: Tensor) -> Tensor: # (bs, c_in, 8, h, w)

        weight_list = []

        top, bot = self.weight.split(4, dim = 2) # (c_out, c_in, 4, k, k) * 2

        # find the rotated weight matrices and add them
        for k in range(4):
            weight_list.append(
                torch.cat([
                torch.roll(
                    torch.rot90(top, k=k, dims=(-2, -1)),
                    shifts=k,
                    dims=2
                ),
                torch.roll(
                    torch.rot90(bot, k=k, dims=(-2, -1)),
                    shifts=k,
                    dims=2
                ),
                ], dim = 2) # (c_out, c_in, 8, k, k)
            )

        ref_top = p4m_reflect_action(top, dim = 2)
        ref_top = torch.flip(ref_top, dims = (-1,))
        ref_bot = p4m_reflect_action(bot, dim = 2)
        ref_bot = torch.flip(ref_bot, dims = (-1,))
    
        for k in range(4):
            weight_list.append(
                torch.cat([
                torch.roll(
                    torch.rot90(ref_bot, k=k, dims=(-2, -1)),
                    shifts=k,
                    dims=2
                ),
                torch.roll(
                    torch.rot90(ref_top, k=k, dims=(-2, -1)),
                    shifts=k,
                    dims=2
                ),
                ], dim = 2) # (c_out, c_in, 8, k, k)
        ...
        w_rot = torch.stack(weight_list, dim = 1) # (c_out, 8, c_in, 8, k, k)

        c_out, _, c_in, _, kh, kw = w_rot.shape
        w_flat = w_rot.reshape(c_out * 8, c_in * 8, kh, kw)

        bs, c_in, _, h, w = x.shape
        x_flat = x.reshape(bs, c_in * 8, h, w)

        x = F.conv2d(x_flat, w_flat, bias=None, padding=self.padding, stride=self.stride) # (bs, c_out * 8, h, w)
...
class P4MResidualConvBlock(nn.Module):
...
class P4MResNet(nn.Module):
    ...
    def forward(self, x: Tensor) -> Tensor:
        x = self.stem(x)  # (bs, channel_dims[0], 8, h, w)

        x = self.stages[0](x)  # (bs, channel_dims[0], 8, h, w)

        for stage in self.stages[1:]:
            x = stage(x)  # (bs, next_c, 8, h / 2^k, w / 2^k)
...
class P4MUNetLiftBlock(nn.Module):
    """Lift a Z2 image into P4M features with a residual, time-conditioned block."""
...
class P4MUNetResidualBlock(nn.Module):
    """A P4M residual block with diffusion-style time conditioning."""
...
class P4MUNetDownBlock(nn.Module):
    """Downsample spatially and refine with a residual P4M block."""
    ...
    def __init__(
        ...
        *,
        ...
        super().__init__()
        self.downsample = P4MP4MConvBlock(
            in_channels,
            out_channels,
            stride=2,
            norm=norm,
        ...
        self.refine = P4MUNetResidualBlock(
            out_channels,
            out_channels,
...
class P4MUNetUpBlock(nn.Module):
    ...
    def __init__(
        ...
        *,
        ...
        self.fuse = P4MUNetResidualBlock(
    ...
    def forward(
        ...
        x = upsample_p4m_feature_map(x, size=skip.shape[-2:])
        x = self.up_conv(x)
        x = torch.cat([skip, x], dim=1)
        return self.fuse(x, time_embedding)


class P4MUNet(nn.Module):
    """A P4M-equivariant U-Net with time conditioning and Z2 output projection."""
    ...
    def __init__(
        ...
        *,
        ...
        self.bottleneck = P4MUNetResidualBlock(
            channel_dims[-1],
            bottleneck_channels,
            time_embedding_dim=time_embedding_dim,
            norm=norm,
